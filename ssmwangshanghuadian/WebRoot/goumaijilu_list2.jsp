...
-->
   ...
   <td align='center'>${u.iszf} <c:if test="${u.iszf=='否'}"><input type="button" value="支付" onClick="zhifu(${u.id},'goumaijilu')"/></c:if> </td>
    <td align='center'>${u.issh} </td>
		 ...
		 <%
			String sql="select * from goumaijilu where goumairen='"+(String)request.getSession().getAttribute("username")+"'  ";
...
if(request.getParameter("tianjiaren")=="" ||request.getParameter("tianjiaren")==null ){}else{String ntianjiaren=new String(request.getParameter("tianjiaren").getBytes("8859_1"));if(ntianjiaren.contains("?")){ntianjiaren=request.getParameter("tianjiaren");}sql=sql+" and tianjiaren like '%"+ntianjiaren+"%'";}
...
if(request.getParameter("jine")=="" ||request.getParameter("jine")==null ){}else{String njine=new String(request.getParameter("jine").getBytes("8859_1"));if(njine.contains("?")){njine=request.getParameter("jine");}sql=sql+" and jine like '%"+njine+"%'";}
